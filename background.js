...
chrome.storage.onChanged.addListener(changes => {
  epoch++;
  // Cached probabilities don't depend on the on/off state, so flipping the switch back on reuses them.
  if (!("enabled" in changes && Object.keys(changes).every(key => key === "enabled" || key === "revision"))) cache.clear();
  pending.clear();
  retryAfter = 0;
  authError = "";
  lastError = "";
